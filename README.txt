...
■ 깃허브(GitHub)로 URL 열기 (선택사항)
  ⚠ 주의: GitHub Pages 무료 호스팅은 "공개" 주소예요.
     링크만 알면 누구나 접속해서 아이들 사진과 이름을 볼 수 있어요.
     검색엔진에 안 걸리게 robots.txt는 넣어뒀지만, 그래도 완전 비공개는
     아니라는 점은 꼭 알고 계셔야 해요.

  1. github.com 에서 새 저장소(Repository)를 만듭니다. (Public)
  2. 이 폴더 안의 모든 파일(index.html, style.css, script.js,
     images 폴더, robots.txt)을 저장소에 업로드합니다.
     (저장소 페이지에서 "Add file → Upload files"로 드래그 앤 드롭 가능)
  3. 저장소의 Settings → Pages 메뉴에서
     Branch를 main, 폴더를 / (root)로 선택하고 저장합니다.
  4. 1~2분 기다리면 https://아이디.github.io/저장소이름/ 주소로
     바로 열립니다.
=======
>>>>>>> b4b4e8d (폴더 구조 정리: root로 이동)
